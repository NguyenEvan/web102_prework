...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        if (games[i].pledged < games[i].goal){
            gameCard.classList.add('unfundedgame-card');
        }
        else{
            gameCard.classList.add('fundedgame-card');
        }
...
let countUnfunded = listOfUnfunded.length;

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `
A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames.toLocaleString()} ${totalGames == 1 ? "game" : "games"}. Currently, ${countUnfunded.toLocaleString()} ${totalGames == 1 ? "game" : "games"} remained unfunded. We need your help to fund these amazing games!
`;

// create a new DOM element containing the template string and append it to the description container
const templateString = document.createElement('p');
templateString.innerText = displayStr;
...
topPledgedGame.innerText = `${firstGame.name.toLocaleString()}`;
firstGameContainer.appendChild(topPledgedGame);
const secondPledgedGame = document.createElement('h1');
secondPledgedGame.innerText = `${secondGame.name.toLocaleString()}`;
